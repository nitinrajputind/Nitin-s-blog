...
const updateUser = async (req, res, next) => {
    if(req.user.id !== req.params.userId){
        return next(errorHandler(403, 'You are not allowed to allow to update this user'))
    }
    ...
    if(req.body.username){
        if(req.body.username.length < 5 ||  req.body.username.length > 20){
            return next(errorHandler(403, 'Username must be in between 5 to 20 characters'));
        }
        ...
        if(!req.body.username.match(/^[a-zA-Z0-9]+$/)){
            return next(errorHandler(403, 'Username only contains letters and numbers'));
        }
    }
    try{
